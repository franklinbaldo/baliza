  ...
  <div class="card modernist-card">
    ...
    {:else if locationStatus === 'ready'}
      <div class="results" in:fade>
        ...
        {#if bids.length === 0}
          <p class="empty">Nenhuma contratação recente encontrada no PNCP para este município.</p>
        {:else}
          <div class="bids-list">
            {#each bids as bid (bid.numeroControlePNCP)}
              <a href={`/baliza/contratacao?id=${bid.numeroControlePNCP}`} class="bid-card" in:fade>
                <div class="bid-meta">
    ...
    {:else if locationStatus === 'error'}
      <p class="error">Não conseguimos determinar sua localização ou o município não foi encontrado.</p>
    {/if}
  </div>
</section>
...
<style>
  ...
  .card-header { display: flex; gap: var(--space-md); margin-bottom: var(--space-lg); }
  ...
  .status-msg { display: flex; align-items: center; gap: 1rem; color: var(--color-secondary); }
  .spinner { width: 24px; height: 24px; border: 3px solid var(--color-base-300); border-top-color: var(--color-primary); border-radius: 50%; animation: spin 1s linear infinite; }
  @keyframes spin { to { transform: rotate(360deg); } }

  .results-header { margin-bottom: var(--space-md); padding-bottom: 8px; border-bottom: 1px solid var(--color-base-200); }
  .bids-list { display: grid; gap: var(--space-sm); }
  .bid-card { 
    display: block; text-decoration: none; color: inherit; padding: var(--space-md); 
    background: var(--color-base-200); border-radius: var(--radius-sm); transition: background 0.2s;
  }
  .bid-card:hover { background: var(--color-base-300); }
  .bid-meta { display: flex; justify-content: space-between; margin-bottom: 4px; }
