...
class SplitTableMigrator:
    ...
    def connect_database(self):
        ...
        try:
            ...
            if legacy_count == 0:
                console.print("⚠️ [yellow]No records in psa.pncp_raw_responses to migrate[/yellow]")
                return False
    ...
    def analyze_existing_data(self):
        ...
        console.print("🔍 [bold blue]Analyzing existing data for deduplication potential[/bold blue]")
        ...
        estimated_savings = estimate_deduplication_savings(unique * avg_size, total_size)
        
        console.print(f"📈 [bold]Sample Analysis Results[/bold] (from {sample_size:,} records):")
        ...
        console.print(f"  Estimated storage savings: {estimated_savings['savings_percentage']:.1f}%")
    ...
    def create_migration_tables(self):
        ...
        if tables_exist != 2:
            console.print("❌ [red]Split tables don't exist. Run extraction first to create schema.[/red]")
            return False
    ...
    def migrate_data_batch(self, offset: int, batch_size: int) -> Dict:
        ...
        for record in legacy_records:
            (legacy_id, extracted_at, endpoint_url, endpoint_name, request_parameters,
             response_code, response_content, response_headers, data_date, run_id,
             total_records, total_pages, current_page, page_size) = record
            ...
            if not self.dry_run:
                self.conn.execute(
                    ...
                    [
                        extracted_at, endpoint_url, endpoint_name, request_parameters,
                        response_code, response_headers, data_date, run_id,
                        total_records, total_pages, current_page, page_size, existing_content_id
                    ]
                )
    ...
    def run_migration(self):
        ...
        console.print("🚀 [bold blue]Starting Migration to Split Table Architecture[/bold blue]")
        ...
        if total_requests_created > 0:
            self.stats['deduplication_rate'] = (total_duplicates_found / total_requests_created) * 100
    ...
    def print_migration_summary(self):
        ...
        console.print(f"💾 **Content Records Created**: {stats['content_records_created']:,}")
        console.print(f"📨 **Request Records Created**: {stats['request_records_created']:,}")
        ...
        if not self.dry_run:
            ...
            try:
                ...
                if theoretical_size and actual_size:
                    ...
                    console.print(f"  Without deduplication: {theoretical_size:,} bytes")
                    console.print(f"  Storage savings: {savings:,} bytes ({savings_pct:.1f}%)")
                    console.print(f"  Compression ratio: {actual_size/theoretical_size:.3f}")
        ...
        if self.dry_run:
            console.print()
            console.print("ℹ️ [dim]This was a dry run - no data was actually migrated[/dim]")
            console.print("Run without --dry-run to perform actual migration")

def main():
    """Main migration script entry point."""
    parser = argparse.ArgumentParser(description="Migrate BALIZA database to split table architecture")
    parser.add_argument(
        "--dry-run", 
        action="store_true", 
