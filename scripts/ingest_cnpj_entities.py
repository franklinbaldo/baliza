            ...
            if response.status_code == 200:
                ...
                entity = {
                    ...
                    "natureza_juridica": str(
                        data.get("company", {}).get("nature", {}).get("code", "")
                    ),
...
def identify_public_entities(cnpj_file: Path) -> set[str]:
    ...
    public_codes = {
        ...
        "1171",
        "1172",
        "1173",  # Fundações Públicas
    }

    public_cnpjs = set()

    with open(cnpj_file, encoding="utf-8") as f:
        reader = csv.DictReader(f)
        for row in reader:
            if row["natureza_juridica"] in public_codes:
                # Clean CNPJ format
                cnpj_clean = (
                    row["cnpj"].replace(".", "").replace("/", "").replace("-", "")
                )
...
def enhance_with_baliza_data(
    public_entities: set[str], baliza_db_path: Path
) -> dict[str, Any]:
    """
    ...
    if not baliza_db_path.exists():
        print(
            f"⚠️ Baliza database not found at {baliza_db_path}, using basic entity list"
        )
        return {cnpj: {"found_in_baliza": False} for cnpj in public_entities}

    print("🔍 Enhancing entities data with Baliza database...")
    ...
    for cnpj in public_entities:
        ...
        if matching_entity:
            ...
            enhanced_data[cnpj] = {"found_in_baliza": False}
...
def main():
    ...
    try:
        ...
        found_in_baliza = sum(
            1 for data in enhanced_data.values() if data["found_in_baliza"]
        )
        print(
            f"📊 {found_in_baliza}/{len(public_entities)} entities found in Baliza data"
        )
