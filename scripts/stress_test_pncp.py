...
class PNCPStressTester:
    ...
    async def test_configuration(self, config: TestConfig) -> TestResult:
        ...
        console.print(f"🧪 Testing: concurrency={config.concurrency}, delay={config.rate_limit_delay}s")
        ...
        async with httpx.AsyncClient(
            ...
            while time.time() < end_time:
                ...
                task = asyncio.create_task(
                    self._make_request(client, endpoint, semaphore, config.rate_limit_delay)
                )
                tasks.append(task)
                endpoint_cycle += 1
    ...
    async def _make_request(self, client: httpx.AsyncClient, endpoint: str, 
                          semaphore: asyncio.Semaphore, rate_limit_delay: float) -> Dict[str, Any]:
    ...
    def _analyze_results(self, config: TestConfig, results: List[Dict], duration: float) -> TestResult:
    ...
    async def run_stress_tests(self) -> List[TestResult]:
        ...
        console.print("Strategy: START AGGRESSIVE (guaranteed 429s) → work backwards to find optimal limits")
        console.print()
        ...
        extreme_configs = [
            TestConfig(concurrency=20, rate_limit_delay=0.0, test_duration=15),  # Maximum pressure
            TestConfig(concurrency=15, rate_limit_delay=0.0, test_duration=15),  # Very high pressure
            TestConfig(concurrency=10, rate_limit_delay=0.0, test_duration=15),  # High pressure, no delay
            TestConfig(concurrency=10, rate_limit_delay=0.05, test_duration=15), # High pressure, minimal delay
            TestConfig(concurrency=8, rate_limit_delay=0.0, test_duration=15),   # High concurrency, no delay
        ]
        ...
        test_configs = extreme_configs + high_pressure_configs + medium_configs + working_back_configs
        
        console.print(f"📊 Running {len(test_configs)} test configurations...")
        console.print("Each test runs for 15 seconds (shorter duration for faster iteration)")
        console.print()
        ...
        with Progress(
            ...
            for i, config in enumerate(test_configs):
                progress.update(task, description=f"Test {i+1}/{len(test_configs)}: c={config.concurrency}, d={config.rate_limit_delay}s")
    ...
    def analyze_and_report(self):
        ...
        for result in sorted(valid_results, key=lambda x: x.success_rate, reverse=True):
            status = "✅ Excellent" if result.success_rate >= 95 and result.rate_limit_errors == 0 else \
                    "🟡 Good" if result.success_rate >= 90 and result.rate_limit_errors <= 5 else \
                    "🔴 Poor"
        ...
        excellent_configs = [r for r in valid_results if r.success_rate >= 95 and r.rate_limit_errors == 0]
        
        if excellent_configs:
            # Sort by throughput (requests per second)
            best_throughput = max(excellent_configs, key=lambda x: x.requests_per_second)
            ...
            console.print()
            console.print(f"🚀 **Best Throughput**: concurrency={best_throughput.concurrency}, delay={best_throughput.rate_limit_delay}s")
            console.print(f"   └── {best_throughput.requests_per_second:.2f} req/s, {best_throughput.success_rate:.1f}% success rate")
            console.print()
            console.print(f"⚡ **Best Latency**: concurrency={best_latency.concurrency}, delay={best_latency.rate_limit_delay}s")
            console.print(f"   └── {best_latency.avg_response_time:.3f}s avg response, {best_latency.success_rate:.1f}% success rate")
            console.print()
            
            # Balanced recommendation
            balanced_configs = sorted(excellent_configs, key=lambda x: x.requests_per_second * (100 - x.avg_response_time * 100), reverse=True)
            if balanced_configs:
                balanced = balanced_configs[0]
                console.print(f"⚖️ **Balanced (Recommended)**: concurrency={balanced.concurrency}, delay={balanced.rate_limit_delay}s")
                console.print(f"   └── {balanced.requests_per_second:.2f} req/s, {balanced.avg_response_time:.3f}s response, {balanced.success_rate:.1f}% success")
        else:
            console.print("⚠️ [yellow]No configurations achieved 95%+ success rate with 0 rate limit errors[/yellow]")
            # Find best available
            best_available = max(valid_results, key=lambda x: x.success_rate)
            console.print(f"📊 **Best Available**: concurrency={best_available.concurrency}, delay={best_available.rate_limit_delay}s")
            console.print(f"   └── {best_available.success_rate:.1f}% success rate, {best_available.rate_limit_errors} rate limit errors")
