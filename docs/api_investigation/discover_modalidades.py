...
async def discover_modalidades():
    ...
    for modalidade_id in modalidades_to_test:
        ...
        if result["success"]:
            if result.get("has_data", False):
                print(
                    f"   ✅ Modalidade {modalidade_id}: {result['total_records']} registros"
                )
                valid_modalidades.append(modalidade_id)
            else:
                print(
                    f"   ⚠️  Modalidade {modalidade_id}: Válida mas sem dados para esta data"
                )
                valid_modalidades.append(modalidade_id)
        else:
            if result.get("status_code") == 422:
                ...
                print(
                    f"   💥 Modalidade {modalidade_id}: Erro {result.get('status_code', 'unknown')}"
                )
    ...
    print("💾 Resultados salvos em: modalidades_discovered.json")
