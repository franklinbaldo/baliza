...
def benchmark_deserialization(payload: Dict[str, Any], iterations: int = 1000):
    ...
    print(f"   📊 vs JSON+zlib: {json_deserial_time/msgpack_deserial_time:.2f}x faster")
    print()
    ...
    print(f"   📊 vs JSON+zlib: {json_deserial_time/msgpack_zlib_deserial_time:.2f}x faster")
    print()
...
def analyze_hash_consistency():
    ...
    print(f"Hashes match: {'✅ Yes' if json_hash == msgpack_hash else '❌ No (expected)'}")
    print()
    ...
    print(f"JSON hash consistency: {'✅ Consistent' if len(hashes_json) == 1 else '❌ Inconsistent'}")
    print(f"MessagePack hash consistency: {'✅ Consistent' if len(hashes_msgpack) == 1 else '❌ Inconsistent'}")
    print()
...
def calculate_storage_savings(current_size_mb: float, compression_ratio: float):
    ...
    print(f"💾 Storage Impact Analysis")
    print("=" * 60)
...
def main():
    ...
    deserialization_results = benchmark_deserialization(payload, iterations=1000)
    ...
    best_compression_ratio = serialization_results["json_zlib"]["avg_size"] / serialization_results["msgpack_zlib"]["avg_size"]
    ...
    if msgpack_zlib_size < json_zlib_size * 0.9 and msgpack_zlib_time <= json_zlib_time * 1.2:
        print("✅ RECOMMENDED: Switch to MessagePack + zlib")
        print(f"   📦 Size reduction: {size_improvement:.1f}x smaller")
        print(f"   ⚡ Performance impact: Acceptable")
        print(f"   🔄 Migration effort: Medium (need to handle existing data)")
    elif msgpack_zlib_size < json_zlib_size * 0.95:
        print("⚠️  CONDITIONAL: Consider MessagePack + zlib")
        print(f"   📦 Size reduction: {size_improvement:.1f}x smaller")
        print(f"   ⚡ Performance: Need to evaluate in production")
    else:
        print("❌ NOT RECOMMENDED: Stick with current JSON + zlib")
        print(f"   📦 Size difference: Not significant enough")
        print(f"   ⚡ Migration cost: Not justified")
