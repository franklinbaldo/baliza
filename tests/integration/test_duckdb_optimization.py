...
def populate_db(extractor):
    # Insert some dummy data directly
...
def test_cleanup_uploaded(extractor, temp_db):
    ...
    with duckdb.connect(str(temp_db)) as con:
        ...
        ids = con.execute(f"SELECT numeroControlePNCP FROM {extractor.dataset}.contratos ORDER BY numeroControlePNCP").fetchall()
...
def test_get_dates_ready_for_export(extractor, temp_db):
    ...
    # We need to mock "now" or use dates far in the past.
    # The populated dates are Jan 2023, so they should be ready if "now" is later.

    dates = extractor.get_dates_ready_for_export(stability_days=1)
    # Depending on when this test runs, 2023 is definitely > 1 day ago.
    # Assuming "now" > 2023.
