...
from baliza.main import _init_baliza_db, _ensure_ia_federation_updated, _check_existing_data_in_ia
    ...
    return [
        {
            ...
            'parquet_urls': ['https://archive.org/download/pncp-contratos-2024-01-01/pncp-contratos-2024-01-01.parquet'],
            'data_date': '2024-01-01',
            'metadata': {'date': '2024-01-01', 'title': 'PNCP Contratos 2024-01-01'}
        },
        {
            'identifier': 'pncp-contratos-2024-01-02',
            'parquet_urls': ['https://archive.org/download/pncp-contratos-2024-01-02/pncp-contratos-2024-01-02.parquet'],
    ...
    return [
        {
            ...
            'orgaoEntidade': {'razaoSocial': 'Prefeitura Municipal', 'cnpj': '11111111000111', 'uf': 'RO'},
        ...
        },
        {
            ...
            'orgaoEntidade': {'razaoSocial': 'Governo do Estado', 'cnpj': '22222222000222', 'uf': 'RO'},
            'data_source': 'internet_archive'
        }
    ]
...
class TestArchiveFirstFlow:
    ...
    def test_complete_archive_first_initialization(self, end_to_end_workspace, mock_ia_data):
        """Test complete archive-first initialization process."""
        with patch('baliza.ia_federation.InternetArchiveFederation.discover_ia_items') as mock_discover, \
             patch('baliza.ia_federation.InternetArchiveFederation.create_federated_views') as mock_create:
    ...
    def test_ia_data_availability_check(self, end_to_end_workspace, mock_ia_data):
        ...
        with patch('baliza.ia_federation.InternetArchiveFederation.discover_ia_items') as mock_discover:
            ...
            with patch('duckdb.connect') as mock_connect:
                ...
                mock_conn.execute.return_value.fetchone.return_value = (1,)  # Data exists
    ...
    def test_federation_discovery_and_cataloging(self, mock_search, end_to_end_workspace):
        """Test IA data discovery and cataloging process."""
        # Mock IA search response
        mock_item1 = Mock()
        mock_item1.identifier = 'pncp-contratos-2024-01-01'
        mock_item1.files = [{'name': 'pncp-contratos-2024-01-01.parquet', 'format': 'Parquet'}]
    ...
    def test_federated_view_creation_with_real_structure(self, end_to_end_workspace, mock_ia_data):
        ...
        with patch.object(federation, 'discover_ia_items', return_value=mock_ia_data):
            ...
            with patch('duckdb.connect') as mock_connect:
                ...
                view_creation_calls = [call for call in mock_conn.execute.call_args_list 
                                     if 'CREATE OR REPLACE VIEW' in str(call)]
                assert len(view_creation_calls) > 0
    
    def test_unified_data_access_priority(self, end_to_end_workspace, mock_parquet_data):
    ...
    def test_archive_first_data_pipeline_integration(self, end_to_end_workspace, mock_ia_data, mock_parquet_data):
        """Test complete archive-first data pipeline integration."""
        _init_baliza_db()
        
        # Mock the complete flow
        with patch('baliza.ia_federation.InternetArchiveFederation.discover_ia_items') as mock_discover, \
             patch('baliza.ia_federation.InternetArchiveFederation.get_data_availability') as mock_availability:
            ...
            assert availability['internet_archive']['total_records'] > availability['local_storage']['total_records']
    ...
    def test_performance_optimization_with_federation(self, end_to_end_workspace):
        ...
        conn.execute("CREATE INDEX idx_unified_date ON federated.contratos_unified(data_date)")
        conn.execute("CREATE INDEX idx_unified_numero ON federated.contratos_unified(numeroControlePncpCompra)")
        ...
        new_ia_data = [
            {
                ...
                'parquet_urls': ['https://archive.org/download/pncp-contratos-2024-01-15/file.parquet'],
    ...
    def test_error_recovery_in_archive_first_flow(self, end_to_end_workspace):
        ...
        with patch.object(federation, 'discover_ia_items', side_effect=Exception("Network timeout")):
...
class TestArchiveFirstStrategy:
    ...
    def test_federation_update_frequency_control(self, end_to_end_workspace):
        ...
        with patch('baliza.ia_federation.InternetArchiveFederation.update_federation') as mock_update:
            _ensure_ia_federation_updated()
    ...
    def test_graceful_degradation_when_ia_unavailable(self, end_to_end_workspace):
        ...
        with patch('internetarchive.search', side_effect=Exception("IA service unavailable")):
...
def test_complete_baliza_archive_first_workflow(end_to_end_workspace):
    ...
    for schema in expected_schemas:
        assert schema in all_schema_names, f"Missing schema in final verification: {schema}"
