...
def test_coverage_temporal_model_syntax(dbt_project_path):
    ...
    found_temporal = sum(
        1 for keyword in temporal_keywords if keyword in model_sql.lower()
    )
...
def test_coverage_entidades_model_syntax(dbt_project_path):
    ...
    found_entity = sum(1 for keyword in entity_keywords if keyword in model_sql.lower())
...
def test_dbt_compile_models(mock_run, dbt_project_path, temp_dbt_workspace):
    ...
    mock_run.return_value = Mock(
        returncode=0, stdout="Compiled successfully", stderr=""
    )
    ...
    for model in coverage_models:
        ...
        result = subprocess.run(
            ["dbt", "compile", "--select", model], capture_output=True, text=True
        )
...
def test_dbt_model_dependencies(dbt_project_path):
    ...
    for model_file in models_dir.rglob("*.sql"):
        ...
        if "select" in model_content.lower():
            uses_dbt_functions = (
                "source(" in model_content
                or "ref(" in model_content
                or "federated." in model_content
            )
...
def test_coverage_metrics_calculation():
    ...
    publishing_entities = sum(
        1 for entity, data in entity_scores.items() if data["contracts"] > 0
    )
...
def test_model_documentation_completeness(dbt_project_path):
    ...
    assert documentation_ratio >= 0.5, (
        f"Insufficient model documentation: {documentation_ratio:.1%}"
    )
