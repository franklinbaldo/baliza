...
async def test_e2e_mcp_sql_query_security():
    ...
    for query in dangerous_queries:
        ...
        assert any(word in error_msg for word in ['not allowed', 'forbidden', 'denied', 'select only']), \
...
async def test_e2e_mcp_complex_query():
    ...
    try:
        ...
        query_result = json.loads(result)
        assert isinstance(query_result, dict), "Complex query should return valid result"
        ...
        expected_columns = ['endpoint_name', 'task_count', 'completed_tasks', 'avg_pages']
