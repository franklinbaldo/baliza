...
async def test_e2e_database_schema():
    ...
    if BALIZA_DB_PATH.exists():
        ...
        required_task_columns = [
            'task_id', 'endpoint_name', 'data_date', 'modalidade',
            'status', 'total_pages', 'missing_pages', 'created_at', 'updated_at'
        ]
        
        for col in required_task_columns:
            assert col in task_column_names, f"Required column {col} missing from tasks table"
        ...
        required_response_columns = [
            'id', 'endpoint_name', 'data_date', 'current_page',
            'response_content', 'extracted_at'
        ]
