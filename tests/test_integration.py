...
def test_complete_data_pipeline(
    mock_fetch, mock_upload, temp_workspace, mock_pncp_response
):
...
def test_monthly_parquet_file_creation(temp_workspace, mock_pncp_response):
    ...
    with (
        patch.dict(os.environ, {"IA_ACCESS_KEY": "test", "IA_SECRET_KEY": "test"}),
        patch("baliza.main.upload"),
    ):
        process_and_upload_data(
            test_date, endpoint_key, endpoint_cfg, test_records, run_summary
        )
