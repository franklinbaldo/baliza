...
def test_notebook_ia_data_access(notebook_path):
    ...
    ia_patterns = ["archive.org", "internetarchive", "baliza", "pncp"]

    for pattern in ia_patterns:
        assert pattern.lower() in notebook_source.lower(), (
            f"Missing IA pattern: {pattern}"
        )
...
def test_notebook_fraud_detection(notebook_path):
    ...
    fraud_keywords = ["fraud", "anomaly", "outlier", "suspicious", "detect"]

    found_keywords = sum(
        1 for keyword in fraud_keywords if keyword.lower() in notebook_source.lower()
    )
...
def test_notebook_visualization_functions(notebook_path):
    ...
    viz_patterns = ["plotly", "plot", "chart", "graph", "visualization"]

    found_patterns = sum(
        1 for pattern in viz_patterns if pattern.lower() in notebook_source.lower()
    )
...
def test_notebook_data_loading_function(notebook_path):
    ...
    assert (
        "def load_sample_data" in notebook_source
        or "load_sample_data" in notebook_source
    )
...
def test_notebook_fallback_data(mock_get, notebook_path):
    ...
    found_patterns = sum(
        1 for pattern in fallback_patterns if pattern.lower() in notebook_source.lower()
    )
...
def test_notebook_error_handling(notebook_path):
    ...
    error_patterns = ["try:", "except:", "except ", "finally:", "raise", "print("]

    found_patterns = sum(1 for pattern in error_patterns if pattern in notebook_source)
...
def test_notebook_data_analysis_completeness(notebook_path):
    ...
    found_components = sum(
        1 for component in analysis_components if component in notebook_source
    )
