...
def openapi_spec():
    ...
    if not os.path.exists(SPEC_FILE_PATH):
        pytest.fail(
            f"OpenAPI spec file not found at {SPEC_FILE_PATH}. Run scripts/fetch_openapi_spec.py first."
        )
...
def test_endpoint_parameters_against_spec(openapi_spec):
    ...
    for endpoint_key, config in ENDPOINTS_CONFIG.items():
        ...
        tamanho_pagina_spec = next(
            (p for p in spec_params if p["name"] == "tamanhoPagina"), None
        )
        if (
            tamanho_pagina_spec
            and "schema" in tamanho_pagina_spec
            and "maximum" in tamanho_pagina_spec["schema"]
        ):
...
def test_required_params_in_spec_are_handled(openapi_spec):
    ...
    for endpoint_key, config in ENDPOINTS_CONFIG.items():
        ...
        for spec_param in spec_params:
            ...
            if (
                spec_param.get("required", False)
                and param_name not in configured_param_names
            ):
                # Check if this parameter is intentionally omitted for this specific endpoint
                if (
                    endpoint_key in intentionally_omitted_for_endpoint
                    and param_name in intentionally_omitted_for_endpoint[endpoint_key]
                ):
                    # Optionally, log this as a known omission rather than failing
                    print(
                        f"INFO: Mandatory parameter '{param_name}' for endpoint '{endpoint_key}' is intentionally omitted."
                    )
                    continue
