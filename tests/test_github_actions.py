...
def workflow_path():
    ...
    return Path(__file__).parent.parent / ".github" / "workflows" / "baliza_daily_run.yml"
...
def test_workflow_dependency_installation(workflow_path):
    ...
    for step in steps:
        if isinstance(step, dict):
            ...
            name = step.get('name', '')
            if any(keyword in run_command.lower() or keyword in name.lower() 
...
def test_workflow_artifact_handling(workflow_path):
    ...
    for step in steps:
        if isinstance(step, dict):
            ...
            run_command = step.get('run', '')
            if any(keyword in run_command.lower() 
...
def test_workflow_schedule_timing(workflow_path):
    ...
    if isinstance(triggers, dict) and 'schedule' in triggers:
        ...
        assert is_off_peak or hour == '*', f"Workflow scheduled during peak hours: {hour}:00 UTC"
    ...
    if python_setup_steps:
        ...
        if 'with' in python_step and 'python-version' in python_step['with']:
            ...
            if version_str.replace('.', '').isdigit():
                ...
                assert major >= 3 and minor >= 8, f"Python version too old: {python_version}"
