...
class TestPipelineConfig:
    ...
    def test_process_resource_config(self):
        ...
        assert (
            processed["endpoint"]["params"]["dataInicial"] == "{incremental.start_date}"
        )
    ...
    def test_generate_modalidade_resources(self):
        ...
        base_resource = {
            ...
            "endpoint": {
                ...
                "params": {"dataInicial": "2024-01-01", "pagina": 1},
            },
        }
        ...
        modalidades = [
            r["endpoint"]["params"]["codigoModalidadeContratacao"]
            for r in modalidade_resources
        ]
...
class TestPipelineSource:
    ...
    def test_baliza_source_creation(
        self, mock_yaml_load, mock_open, mock_rest_api_source
    ):
        """Testa criação do source Baliza."""
        # Mock da configuração YAML
        mock_config = {
            "sync_resources": [
                {"name": "test_resource", "endpoint": {"params": {"pagina": 1}}}
            ]
        }
        ...
        source = baliza_source(base_url="https://test.api.com", resource_type="sync")
...
class TestPipelineDryRun:
    ...
    def test_pipeline_dry_run_sync(self, mock_dlt_pipeline, mock_baliza_source):
        ...
        # Executa dry run
        result = run_pipeline(
            destination="duckdb", dataset_name="test_data", resource_type="sync"
        )
    ...
    def test_pipeline_config_validation(self):
        ...
        try:
            ...
            source = baliza_source(
                base_url="https://pncp.gov.br/api/consulta", resource_type="sync"
            )
