...
def test_get_data_availability_empty(federation, temp_db):
    ...
    with patch.object(federation, "discover_ia_items", return_value=[]):
        ...
        assert availability["internet_archive"]["date_range"] == {
            "min_date": None,
            "max_date": None,
        }
...
def test_update_federation(federation):
    ...
    with (
        patch.object(federation, "discover_ia_items") as mock_discover,
...
def test_federation_error_handling(federation):
    ...
    with patch.object(
        federation, "discover_ia_items", side_effect=Exception("IA API Error")
    ):
...
@pytest.mark.parametrize(
    ...
    [(404, "IA item not found"), (500, "IA server error"), (403, "Access denied")],
)
