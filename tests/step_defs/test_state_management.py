...
def run_state_show(db_path_with_windows: Path):
    """Run the real 'state show' command."""
...
def run_state_gaps(db_path_with_windows: Path):
    """Run the real 'state gaps' command."""
    result = runner.invoke(
        app,
        [
            "verify",  # Using verify to have end date control
            "--start",
            "2024-01-01",
            "--end",
            "2024-01-04",
...
def run_state_history(db_path_with_history: Path):
    """Run the real 'state history' command."""
...
def check_extraction_history(result):
    ...
    assert "run-1" in result.stdout or "run-1"[:8] in result.stdout
    assert "completed" in result.stdout
    assert "run-2" in result.stdout or "run-2"[:8] in result.stdout
