...
def db_with_contracts(tmp_path: Path, date_str: str) -> dict:
    ...
    with duckdb.connect(str(db_file)) as con:
        ...
        # Insert 50 contracts for the date
        for i in range(50):
            con.execute(
                """
                INSERT INTO baliza_raw.contratos VALUES (
                    ?, 2023, ?, '12345678000190', 'Test Org', 'EXECUTIVO',
                    '001', 'Unit 1', 1, 'Pregão', 1000.00 + ?,
                    ?, NULL, NULL, 'Test contract', NULL, 'PROC-001',
                    'http://example.com', ?, ?, 'Test User'
                )
            """,
                [
                    f"CTRL-{i:05d}",
...
def daily_export(tmp_path: Path, date_str: str) -> dict:
    ...
    with duckdb.connect(str(db_file)) as con:
        ...
        con.execute(
            f"""
            INSERT INTO baliza_raw.contratos VALUES (
                'CTRL-00001', 2023, 1, '12345678000190', 'Test Org', '001',
                1000.00, '{date_str}T10:00:00', '{date_str}T10:00:00',
                '{date_str}T10:00:00', 'Test', NULL, 'PROC-001',
                'http://example.com', NULL, NULL, 'User', 1, 'Pregão',
                'EXECUTIVO', 'Unit 1'
            )
        """
        )
    ...
    # Run export
    exporter = DailyExporter(db_file, "baliza_raw")
...
def db_with_multiple_orgs(tmp_path: Path, contract_count: int, org_count: int) -> dict:
    ...
    with duckdb.connect(str(db_file)) as con:
        ...
        # Create contracts distributed across orgs
        for i in range(contract_count):
            org_index = i % org_count
            cnpj = f"{org_index:014d}"
            con.execute(
                """
                INSERT INTO baliza_raw.contratos VALUES (
                    ?, ?, ?, 'EXECUTIVO', 1000.00, '2023-01-15T10:00:00',
                    2023, ?, NULL, NULL, 1, 'Pregão', 'Test', NULL, 'PROC',
                    'http://example.com', '2023-01-15T10:00:00',
                    '2023-01-15T10:00:00', NULL, NULL, 'User'
                )
            """,
                [f"CTRL-{i:05d}", cnpj, f"Org {org_index}", i],
            )
...
def join_contratos_orgaos(daily_export):
    ...
    with duckdb.connect(":memory:") as con:
        result = con.execute(
            f"""
            SELECT c.numero_controle_pncp, c.orgao_cnpj, o.cnpj
            FROM '{contratos_file}' c
            LEFT JOIN '{orgaos_file}' o ON c.orgao_cnpj = o.cnpj
        """
        ).fetchall()
...
def check_all_contracts_have_orgs(join_result):
    ...
    for row in results:
        numero_controle, orgao_cnpj, org_cnpj = row
        assert org_cnpj is not None, f"Contract {numero_controle} has no matching org"
        assert orgao_cnpj == org_cnpj, f"CNPJ mismatch for {numero_controle}"
    ...
    with duckdb.connect(str(db_file)) as con:
        ...
        for i in range(count):
            con.execute(
                """
                INSERT INTO baliza_raw.contratos VALUES (
                    ?, '12345678000190', 'Test Org', 1000.00, '2023-01-15T10:00:00',
                    2023, ?, NULL, 1, 'Pregão', 'Test', NULL, 'PROC',
                    'http://example.com', '2023-01-15T10:00:00',
                    '2023-01-15T10:00:00', NULL, NULL, 'User', 'EXECUTIVO', NULL
                )
            """,
                [f"CTRL-{i:05d}", i],
            )
