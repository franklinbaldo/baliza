...
def test_pncp_api_endpoint(test_date_str):
    ...
    """
    print(
        f"Attempting to connect to PNCP API endpoint: {ENDPOINT_PATH} for date: {test_date_str}\n"
    )
    ...
    headers = {"User-Agent": "baliza-test/0.1.0"}
    ...
    try:
        ...
        assert response.status_code in [200, 204], (
            f"Expected status code 200 or 204, got {response.status_code}. Response: {response.text}"
        )
        if response.status_code == 204:
            print(
                "Received 204 No Content. This is expected if no data for the given date/parameters."
            )
