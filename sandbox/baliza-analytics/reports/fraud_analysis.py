...
def __(mo):
    ...
    date_range = mo.ui.date_range(
        start="2024-01-01", end="2024-12-31", label="📅 Período de Análise"
    )

    min_score_slider = mo.ui.slider(
        start=0, stop=100, value=50, step=5, label="🎯 Score Mínimo de Suspeição"
    )
    ...
    min_value_input = mo.ui.number(value=100000, label="💰 Valor Mínimo do Contrato (R$)")
...
@app.cell
def __(mo):
    load_button = mo.ui.button(value="🔄 Carregar e Analisar Dados", kind="success")
    load_button
    return (load_button,)
...
def __(
    ...
    if load_button.value:
        try:
            ...
            mo.md(f"❌ **Erro ao carregar dados:** {e!s}")
            contracts_df = None
            fraud_report = None
...
def __(contracts_with_scores, fraud_report, mo, px):
    ...
    if fraud_report is not None:
        ...
        risk_df = pd.DataFrame(
            [
                {
                    "Nível de Risco": "Baixo (0-24)",
                    "Quantidade": risk_dist["low_risk"],
                    "Cor": "#2E8B57",
                },
                {
                    "Nível de Risco": "Médio (25-49)",
                    "Quantidade": risk_dist["medium_risk"],
                    "Cor": "#FFA500",
                },
                {
                    "Nível de Risco": "Alto (50-74)",
                    "Quantidade": risk_dist["high_risk"],
                    "Cor": "#FF6347",
                },
                {
                    "Nível de Risco": "Crítico (75-100)",
                    "Quantidade": risk_dist["critical_risk"],
                    "Cor": "#DC143C",
                },
            ]
        )
...
def __(contracts_with_scores, mo, px):
    ...
    if contracts_with_scores is not None:
        ...
        fig_scatter.add_hline(
            y=50, line_dash="dash", line_color="red", annotation_text="Limiar de Suspeição"
        )
...
def __(fraud_report, mo):
    ...
    if fraud_report is not None and fraud_report["top_suspicious"]:
        ...
        display_df.columns = ["Score", "ID do Contrato", "Órgão", "Fornecedor", "Valor (R$ M)"]

        # Round values
        display_df["Valor (R$ M)"] = display_df["Valor (R$ M)"].round(2)

        mo.ui.table(display_df, selection=None, pagination=True, page_size=10)
...
def __(contracts_with_scores, mo, px):
    ...
    if contracts_with_scores is not None:
        ...
        # Monthly aggregation
        monthly_stats = (
            temporal_df.groupby("mes_ano")
            .agg({"contrato_id": "count", "suspeito": "sum", "valor_global_brl": "sum"})
            .reset_index()
        )

        monthly_stats["taxa_suspeicao"] = (
            monthly_stats["suspeito"] / monthly_stats["contrato_id"] * 100
        ).round(2)
        monthly_stats["valor_milhoes"] = monthly_stats["valor_global_brl"] / 1_000_000

        # Create dual-axis chart
        fig_timeline = make_subplots(
            specs=[[{"secondary_y": True}]], subplot_titles=("Evolução Temporal dos Contratos")
        )
