...
class ContractFraudDetector:
    ...
    def calculate_suspicious_score(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if "objeto_contrato" in df.columns:
            ...
            emergency_contracts = (
                df["objeto_contrato"].str.lower().str.contains(emergency_pattern, na=False)
            )
    ...
    def detect_anomalies_ml(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        numerical_features = ["valor_global_brl", "duracao_contrato_dias"]
    ...
    def get_suspicious_contracts(self, df: pd.DataFrame, min_score: int = 50) -> pd.DataFrame:
        """
    ...
    def analyze_supplier_risk(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        """
        supplier_analysis = (
            df.groupby(["fornecedor_ni", "fornecedor_nome"])
            .agg(
                {
                    "contrato_id": "count",
        ...
        supplier_analysis["concentracao_geografica"] = supplier_analysis["ufs_diferentes"] == 1
        supplier_analysis["concentracao_orgaos"] = supplier_analysis["orgaos_diferentes"] <= 2
        supplier_analysis["alto_volume"] = supplier_analysis["total_contratos"] > supplier_analysis[
            "total_contratos"
        ].quantile(0.95)
    ...
    def generate_fraud_report(self, df: pd.DataFrame) -> dict:
        ...
        return {
            ...
            "risk_distribution": {
                ...
                "medium_risk": len(
                    df_analyzed[
                        (df_analyzed["score_suspeita"] >= 25) & (df_analyzed["score_suspeita"] < 50)
                    ]
                ),
