...
@app.command()
def stats(baliza_db: str | None = typer.Option(None, help="Path to Baliza database")):
...
def analyze(
    ...
    uf: str | None = typer.Option(None, help="State filter (e.g., RO,SP)"),
    min_value: float | None = typer.Option(None, help="Minimum contract value"),
    min_score: int = typer.Option(50, help="Minimum suspicious score (0-100)"),
    baliza_db: str | None = typer.Option(None, help="Path to Baliza database"),
):
    ...
    try:
        ...
        df = connector.get_contracts(
            start_date=start_date, end_date=end_date, uf_filter=uf_filter, min_value=min_value
        )
        ...
        if report["top_suspicious"]:
            ...
            for contract in report["top_suspicious"]:
                suspicious_table.add_row(
                    ...
                    contract["contrato_id"][:20] + "..."
                    if len(contract["contrato_id"]) > 20
                    else contract["contrato_id"],
                    contract["orgao_razao_social"][:30] + "..."
                    if len(contract["orgao_razao_social"]) > 30
                    else contract["orgao_razao_social"],
                    contract["fornecedor_nome"][:25] + "..."
                    if len(contract["fornecedor_nome"]) > 25
                    else contract["fornecedor_nome"],
        ...
        for level, count in report["risk_distribution"].items():
            ...
            risk_table.add_row(level.replace("_", " ").title(), f"{count:,}", f"{percentage:.1f}%")
...
def report(
    ...
    try:
        subprocess.run(["marimo", "run", str(report_file), "--port", str(port)], check=True)
...
def suppliers(
    ...
    try:
        ...
        for _, supplier in top_suppliers.iterrows():
            table.add_row(
                supplier["fornecedor_nome"][:40] + "..."
                if len(supplier["fornecedor_nome"]) > 40
                else supplier["fornecedor_nome"],
