...
def state_show(
    ...
    try:
        ...
        def make_row_content(count: int, color: str) -> Table:
            # Using a nested table (grid) to align bar and text
            grid = Table.grid(padding=(0, 1))
            grid.add_column(width=20)  # Fixed width for bar
            grid.add_column(justify="right", width=8)  # Fixed width for pct

            pct_text = "0.0%"
            if total > 0:
                pct_text = f"{(count / total) * 100:.1f}%"
        ...
        table.add_row(
            ...
            str(counts["ok"]),
            make_row_content(counts["ok"], "green"),
        )
        table.add_row(
            "[yellow]Incomplete windows[/yellow]",
            str(counts["incompleto"]),
            make_row_content(counts["incompleto"], "yellow"),
        )
        table.add_row(
            "[red]Suspect windows[/red]",
            str(counts["suspeito"]),
            make_row_content(counts["suspeito"], "red"),
        )
        table.add_row(
            "[cyan]Unprocessed windows[/cyan]",
            str(counts["nao_processado"]),
            make_row_content(counts["nao_processado"], "cyan"),
        )

        # Total row doesn't need a bar
        table.add_section()
