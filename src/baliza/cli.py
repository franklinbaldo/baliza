...
def export(
    ...
    if ia_identifier and ia_access_key and ia_secret_key:
        # Validate ia_identifier to prevent path traversal
        if os.path.sep in ia_identifier or (os.path.altsep and os.path.altsep in ia_identifier):
            typer.secho(
                "Error: ia_identifier cannot contain path separators.",
                fg=typer.colors.RED,
                err=True,
            )
            raise typer.Exit(code=1)
