...
@app.command()
def sync(
    destination: str = typer.Option("duckdb", help="Destino dos dados (duckdb, postgresql, etc.)"),
...
def status():
    ...
    typer.echo(f"Configuração YAML: {'✅' if config_path.exists() else '❌'} {config_path}")
...
def info():
    ...
    """
    from .pipeline import _load_yaml_config, _get_resources_by_type, _requires_modalidade
