...
class _FallbackClient(AbstractContextManager["_FallbackClient"]):
    ...
    def _validate_url_host(self, url: str) -> None:
        """Security: Validate that the host does not resolve to a private/loopback IP."""
        import ipaddress
        import socket
        ...
        try:
            ...
            # Use 0 for port to avoid service lookup overhead
            addr_info = socket.getaddrinfo(hostname, 0)
        except socket.gaierror:
            # If we can't resolve it, let the request fail naturally later
            return
        ...
        for _, _, _, _, sockaddr in addr_info:
            ...
            try:
                ...
                if ip.is_private or ip.is_loopback:
                    raise RuntimeError(
                        f"Security: Host '{hostname}' resolves to private/loopback IP {ip_str}. "
