...
class AsyncPNCPExtractor:
    ...
    def _filter_existing_discoveries(self, plan_df: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            logger.exception("Error fetching existing discoveries from the database.")
            return plan_df
    ...
    async def _discover_total_pages_concurrently(self, initial_plan_df: pd.DataFrame):
        ...
        # Create workers
        workers = [asyncio.create_task(self._process_page_queue(page_queue, total_tasks, True)) for _ in range(self.concurrency)]
    ...
    def _expand_and_reconcile_from_db_vectorized(self, start_date: date, end_date: date) -> pd.DataFrame:
        ...
        try:
            ...
            logger.exception(f"Error during anti-join reconciliation: {e}")
            console.print(f"⚠️ Error during reconciliation: {e}")
            return pd.DataFrame()
    ...
    async def _execute_fetch_tasks(self, tasks_df: pd.DataFrame):
        ...
        # Create workers
        workers = [asyncio.create_task(self._process_page_queue(page_queue, total_tasks, False)) for _ in range(self.concurrency)]
