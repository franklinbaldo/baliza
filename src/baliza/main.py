...
def _init_baliza_db():
    ...
    # PSA: Raw contratações data (procurement processes)
    conn.execute("""
        CREATE TABLE IF NOT EXISTS psa.contratacoes_raw (
            ...
            baliza_modalidade_nome VARCHAR,

            -- PNCP API raw fields for contratações
            numeroControlePNCP VARCHAR,
            anoCompra INTEGER,
            sequencialCompra INTEGER,
            numeroCompra VARCHAR,
            dataInclusao VARCHAR,
            dataPublicacaoPncp VARCHAR,
            dataAtualizacao VARCHAR,
            dataAtualizacaoGlobal VARCHAR,
            processo VARCHAR,
            objetoCompra VARCHAR,
            valorTotalEstimado DOUBLE,
            valorTotalHomologado DOUBLE,
            ...
            orgaoEntidade_json VARCHAR,
            unidadeOrgao_json VARCHAR,
            amparoLegal_json VARCHAR,
            fontesOrcamentarias_json VARCHAR,
    ...
    # PSA: Raw atas data (price registration records)
    conn.execute("""
    ...
    # Contratações indexes
    conn.execute("CREATE INDEX IF NOT EXISTS idx_contratacoes_raw_date ON psa.contratacoes_raw(baliza_data_date)")
    conn.execute("CREATE INDEX IF NOT EXISTS idx_contratacoes_raw_numero ON psa.contratacoes_raw(numeroControlePNCP)")
    conn.execute("CREATE INDEX IF NOT EXISTS idx_contratacoes_raw_modalidade ON psa.contratacoes_raw(baliza_modalidade)")
    
    # Atas indexes  
    conn.execute("CREATE INDEX IF NOT EXISTS idx_atas_raw_date ON psa.atas_raw(baliza_data_date)")
    conn.execute("CREATE INDEX IF NOT EXISTS idx_atas_raw_numero ON psa.atas_raw(numeroControlePNCPAta)")
    conn.execute("CREATE INDEX IF NOT EXISTS idx_atas_raw_orgao ON psa.atas_raw(cnpjOrgao)")
...
# All discovered PNCP endpoints with their required parameters
ENDPOINTS_CONFIG = {
    # Contratos - Original working endpoint
    ...
    "contratos_atualizacao": {
        ...
        "tamanhoPagina": 100,  # Conservative for new endpoint
        "date_param_initial": "dataInicial",
        "date_param_final": "dataFinal", 
        "date_format": "yyyyMMdd",
        "required_params": {},
    ...
    # Contratações by modalidade - High priority endpoints
    "contratacoes_dispensa": {
        "api_path": "/v1/contratacoes/publicacao",
        "file_prefix": "pncp-contratacoes-dispensa",
        "ia_title_prefix": "PNCP Contratações Dispensa",
        "tamanhoPagina": 100,
        "date_param_initial": "dataInicial",
        "date_param_final": "dataFinal",
        "date_format": "yyyyMMdd",
        "required_params": {"codigoModalidadeContratacao": 8},
        "data_type": "contratacoes",
    ...
    "contratacoes_pregao_eletronico": {
        ...
        "ia_title_prefix": "PNCP Contratações Pregão Eletrônico",
        "tamanhoPagina": 100,
        "date_param_initial": "dataInicial",
        "date_param_final": "dataFinal",
        "date_format": "yyyyMMdd",
        "required_params": {"codigoModalidadeContratacao": 6},
        "data_type": "contratacoes",
    ...
    "contratacoes_inexigibilidade": {
        ...
        "ia_title_prefix": "PNCP Contratações Inexigibilidade",
        "tamanhoPagina": 100,
        "date_param_initial": "dataInicial",
        "date_param_final": "dataFinal",
        "date_format": "yyyyMMdd",
        "required_params": {"codigoModalidadeContratacao": 9},
        "data_type": "contratacoes",
    ...
    "contratacoes_concorrencia_eletronica": {
        ...
        "ia_title_prefix": "PNCP Contratações Concorrência Eletrônica", 
        "tamanhoPagina": 100,
        "date_param_initial": "dataInicial",
        "date_param_final": "dataFinal",
        "date_format": "yyyyMMdd",
        "required_params": {"codigoModalidadeContratacao": 4},
        "data_type": "contratacoes",
        "modalidade_nome": "Concorrência Eletrônica",
    },
    
    # Atas - High volume endpoints
    "atas_periodo": {
        "api_path": "/v1/atas",
        "file_prefix": "pncp-atas-periodo",
        "ia_title_prefix": "PNCP Atas Registro Preços",
        "tamanhoPagina": 500,  # Large dataset, use bigger pages
        "date_param_initial": "dataInicial",
        "date_param_final": "dataFinal",
        "date_format": "yyyyMMdd",
        "required_params": {},
