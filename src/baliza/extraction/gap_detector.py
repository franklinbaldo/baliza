...
class DataGap:
    ...
    missing_pages: Optional[List[int]] = (
        None  # Specific pages missing, None = all pages
    )
    ...
    def __str__(self):
        ...
        return (
            f"{self.endpoint}: {self.start_date}-{self.end_date}{modal_str}{pages_str}"
        )
...
class PNCPGapDetector:
    ...
    def find_missing_date_ranges(
        ...
        for endpoint in endpoints:
            if check_pagination:
                endpoint_gaps = self._find_endpoint_and_pagination_gaps(
    ...
    def _find_endpoint_and_pagination_gaps(
    ...
    def _get_existing_requests_with_pagination(self, table) -> Dict[str, Set[int]]:
        ...
        try:
            ...
            query = (
                table.filter(table._dlt_load_id.notnull())
                .select(
                    [
            ...
            for row in results.itertuples():
                if hasattr(row, "_baliza_extracted_at") and row._baliza_extracted_at:
                    try:
                        extract_date = datetime.fromisoformat(
                            row._baliza_extracted_at.replace("Z", "+00:00")
                        ).date()
    ...
    def _find_pagination_gaps(
        self, start_date: str, end_date: str, existing_requests: Dict[str, Set[int]]
        ...
        while current_dt <= end_dt:
            ...
            if date_str in existing_requests:
                ...
                if missing_pages:
                    ...
                    realistic_missing = [
                        p for p in missing_pages if p <= max_existing + 10
                    ]
    ...
    def _get_existing_date_ranges(self, table) -> List[Tuple[date, date]]:
        ...
        try:
            ...
            query = (
                table.filter(table._dlt_load_id.notnull())  # Successfully loaded
    ...
    def _find_gaps_in_ranges(
        self, requested: Tuple[date, date], existing: List[Tuple[date, date]]
    ) -> List[Tuple[date, date]]:
    ...
    def get_backfill_gaps(self, endpoints: List[str] = None) -> List[DataGap]:
        ...
        Returns:
            List of DataGap objects for monthly backfill starting from 2021-01
        """
        if not endpoints:
            endpoints = self.endpoints
