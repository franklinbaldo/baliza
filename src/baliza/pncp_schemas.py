...
class BronzeContrato(BaseModel):
    ...
    @field_validator('valor_inicial', 'valor_parcela', 'valor_global', 'valor_acumulado')
    @classmethod
    ...
    def validate_date_consistency(self) -> 'BronzeContrato':
        ...
        if (self.data_vigencia_inicio and self.data_vigencia_fim and 
            ...
            raise ValueError("Data de início da vigência não pode ser posterior à data de fim")
    ...
    def from_api_response(cls, contrato_data: dict) -> "BronzeContrato":
        ...
        return cls(
            ...
            nome_fornecedor_subcontratado=contrato_data.get("nomeFornecedorSubContratado"),
...
class BronzeContratacao(BaseModel):
    ...
    def validate_date_consistency(self) -> 'BronzeContratacao':
        """Valida consistência entre datas."""
        if (self.data_abertura_proposta and self.data_encerramento_proposta and 
            self.data_abertura_proposta > self.data_encerramento_proposta):
            raise ValueError("Data de abertura não pode ser posterior à data de encerramento")
        
        # Datas não podem ser no futuro distante (> 1 ano)
        future_limit = date.today().replace(year=date.today().year + 1)
        if self.data_abertura_proposta and self.data_abertura_proposta.date() > future_limit:
    ...
    def from_api_response(cls, contratacao_data: dict) -> "BronzeContratacao":
        ...
        return cls(
            ...
            # Instrumento Convocatório
            tipo_instrumento_convocatorio_codigo=contratacao_data.get("tipoInstrumentoConvocatorioCodigo"),
            tipo_instrumento_convocatorio_nome=contratacao_data.get("tipoInstrumentoConvocatorioNome"),
...
class BronzeFonteOrcamentaria(BaseModel):
    ...
    @classmethod
    def from_api_response(cls, fonte_data: dict, numero_controle_pncp: str) -> "BronzeFonteOrcamentaria":
...
class BronzeAta(BaseModel):
    ...
    def validate_date_consistency(self) -> 'BronzeAta':
        ...
        if (self.vigencia_inicio and self.vigencia_fim and 
            self.vigencia_inicio > self.vigencia_fim):
            raise ValueError("Data de início da vigência não pode ser posterior à data de fim")
        
        # Data de assinatura deve ser anterior ao início da vigência
        if (self.data_assinatura and self.vigencia_inicio and 
            self.data_assinatura > self.vigencia_inicio):
            raise ValueError("Data de assinatura deve ser anterior ao início da vigência")
        ...
        if (self.data_cancelamento and self.data_assinatura and 
            self.data_cancelamento < self.data_assinatura):
            raise ValueError("Data de cancelamento deve ser posterior à data de assinatura")
