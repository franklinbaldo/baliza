...
def marts_creation() -> Dict[str, Any]:
    ...
    try:
        ...
        con = connect()
        summary_count = con.raw_sql("SELECT COUNT(*) as cnt FROM marts.extraction_summary").fetchone()[0]
        quality_count = con.raw_sql("SELECT COUNT(*) as cnt FROM marts.data_quality").fetchone()[0]
