...
from ..enums import ModalidadeContratacao, SituacaoContratacao, get_enum_by_value
from .circuit_breaker import CircuitBreaker, CircuitBreakerConfig, AdaptiveRateLimiter, retry_with_backoff, RetryConfig
...
class PNCPClient:
    ...
    async def fetch_endpoint_data(
        ...
        request_metadata = RequestMetadata(
            ...
            date_range=f"{params.get('dataInicial')}-{params.get('dataFinal')}" if params else None
        )
        ...
        async with httpx.AsyncClient(**self.client_config) as client:
            try:
                ...
                if response.status_code == 204 or not response.content:
                    payload_json = {"data": [], "totalRegistros": 0, "totalPaginas": 0, 
                                  "numeroPagina": 1, "paginasRestantes": 0, "empty": True}
                else:
    ...
    def _create_api_request(
        ...
        payload_bytes = json.dumps(payload_json, ensure_ascii=False, separators=(',', ':')).encode('utf-8')
...
class EndpointExtractor:
    ...
    async def extract_contratacoes_publicacao(
        ...
        **filters
        ...
        self.logger.info(
            f"Completed extraction: {len(requests)} pages, "
            f"modalidade {modalidade.name}"
        )
    ...
    async def extract_atas(
        ...
        **filters
        ...
        self.logger.info(
            f"Starting extraction: atas ({data_inicial} to {data_final})"
        )
