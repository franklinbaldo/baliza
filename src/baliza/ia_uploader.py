...
class IAUploader:
        ...
        try:
            with duckdb.connect(":memory:") as con:
                ...
                try:
                    ...
                    df = con.execute(
                        f"SELECT * FROM read_parquet('{MANIFEST_URL}')"
                    ).fetch_arrow_table()
                    ...
                    file_urls, table_names, dates = zip(*rows, strict=False)
                    data = {
    ...
    def get_uploaded_dates(self) -> set[datetime.date]:
        ...
        return {d for d, t in zip(dates, table_names, strict=False) if t == "contratos"}
    ...
    def _append_to_manifest(
        ...
        for table_name, info in export_stats.get("tables", {}).items():
            ...
            new_rows["file_url"].append(f"https://archive.org/download/{item_id}/{fname}")
    ...
    def upload_day(
        ...
        if row_count == 0:
            console.print(
                f"[yellow]{target_date}: 0 contracts — marking in manifest as empty.[/yellow]"
            )
            ...
            self._update_manifest_on_ia(
                target_date, item_id, stats, ia_access_key, ia_secret_key, output_dir
            )
            return True
        ...
        item_id = _ia_item_for_date(target_date)
        upload_dict = {_ia_filename(f.stem, target_date): str(f) for f in parquet_files}
    ...
    def _update_manifest_on_ia(  # noqa: PLR0913
        self,
        target_date: datetime.date,
        item_id: str,
        export_stats: dict,
        ...
        try:
            ...
            console.print(f"[green]✓ Manifest updated ({updated.num_rows} total rows).[/green]")
        except Exception as e:
