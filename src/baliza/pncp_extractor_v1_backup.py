...
class SimplePNCPExtractor:
    ...
    def _get_progress_info(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "rps": self._get_rps(),
            "progress_percent": (
                self.total_pages_processed / self.total_pages_expected * 100
            )
    ...
    def _init_database(self):
        ...
        self.conn.execute(
            "CREATE INDEX IF NOT EXISTS idx_endpoint_url ON psa.pncp_raw_responses(endpoint_url)"
        )
        self.conn.execute(
            "CREATE INDEX IF NOT EXISTS idx_data_date ON psa.pncp_raw_responses(data_date)"
        )
        self.conn.execute(
            "CREATE INDEX IF NOT EXISTS idx_response_code ON psa.pncp_raw_responses(response_code)"
        )
        self.conn.execute(
            "CREATE INDEX IF NOT EXISTS idx_extracted_at ON psa.pncp_raw_responses(extracted_at)"
        )
        self.conn.execute(
            "CREATE INDEX IF NOT EXISTS idx_endpoint_name ON psa.pncp_raw_responses(endpoint_name)"
        )
    ...
    def _check_exact_url_exists(
        ...
        query_params = [
            endpoint_url,
            str(params.get("pagina", 1)),
            str(params.get("tamanhoPagina", 500)),
        ]
    ...
    def _check_existing_extraction_range(
        self,
    ...
    async def _make_request_async(
        ...
        try:
            ...
            if response.status_code >= 400:
                ...
                console.print(
                    f"    ❌ [red]HTTP {response.status_code} - {endpoint['name']}[/red]"
                )
                console.print(
                    f"    🌐 [dim red]Full URL: {full_url}?{param_str}[/dim red]"
                )
                console.print(f"    📋 [dim red]Parameters: {params}[/dim red]")
                if response.text and len(response.text) < 500:
                    console.print(
                        f"    📄 [dim red]Response: {response.text[:200]}[/dim red]"
                    )
    ...
    def _store_response_range(
        ...
        """,
            [
                ...
                self.run_id,
                endpoint["name"].split("_")[
                    0
                ],  # Extract type: contratos, contratacoes, atas, etc.
                modalidade,
    ...
    async def extract_endpoint_date_range(
        self, endpoint: Dict[str, Any], start_date: datetime, end_date: datetime
    ) -> Dict[str, Any]:
        ...
        if "modalidades" in endpoint:
            modalidade_strategy = endpoint.get(
                "modalidade_strategy", "required_individual"
            )

            if modalidade_strategy == "optional_unrestricted":
                # Try unrestricted extraction first (no modalidade parameter)
                console.print(
                    f"    🔍 [blue]Trying unrestricted extraction for {endpoint['name']}[/blue]"
                )
                unrestricted_results = (
                    await self._extract_endpoint_modalidade_range_async(
                        endpoint, start_date, end_date, None
                    )
                )

                if (
                    unrestricted_results["success_requests"] > 0
                    and unrestricted_results["total_records"] > 0
                ):
                    # Unrestricted extraction succeeded and has data
                    console.print(
                        f"    ✅ [green]Unrestricted extraction successful: {unrestricted_results['total_records']:,} records[/green]"
                    )
                    results["total_requests"] += unrestricted_results["total_requests"]
                    results["total_records"] += unrestricted_results["total_records"]
                    results["success_requests"] += unrestricted_results[
                        "success_requests"
                    ]
                    ...
                    console.print(
                        f"    ⚠️ [yellow]Unrestricted extraction failed (success: {unrestricted_results['success_requests']}, records: {unrestricted_results['total_records']}, errors: {unrestricted_results['error_requests']}), falling back to individual modalidades[/yellow]"
                    )
                    await self._process_individual_modalidades(
                        endpoint, start_date, end_date, results
                    )
            else:
            ...
            modalidade_results = await self._extract_endpoint_modalidade_range_async(
                endpoint, start_date, end_date, None
            )
    ...
    async def _process_individual_modalidades(
        self,
        endpoint: Dict[str, Any],
        start_date: datetime,
        end_date: datetime,
        ...
        for modalidade in endpoint["modalidades"]:
            task = self._extract_endpoint_modalidade_range_async(
                endpoint, start_date, end_date, modalidade
            )
            modalidade_tasks.append(task)

        # Wait for all modalidades to complete
        modalidade_results_list = await asyncio.gather(
            *modalidade_tasks, return_exceptions=True
        )
        ...
        for i, modalidade_results in enumerate(modalidade_results_list):
            ...
            if isinstance(modalidade_results, Exception):
                console.print(
                    f"❌ Error processing modalidade {modalidade}: {modalidade_results}"
                )
    ...
    def extract_endpoint_data(
        self, endpoint: Dict[str, Any], data_date: datetime
    ) -> Dict[str, Any]:
        ...
        if "modalidades" in endpoint:
            for modalidade in endpoint["modalidades"]:
                modalidade_results = self._extract_endpoint_modalidade(
                    endpoint, data_date, modalidade
                )
                results["total_requests"] += modalidade_results["total_requests"]
                results["total_records"] += modalidade_results["total_records"]
                results["success_requests"] += modalidade_results["success_requests"]
    ...
    def _extract_endpoint_modalidade_range(
        self,
        endpoint: Dict[str, Any],
        start_date: datetime,
        end_date: datetime,
        modalidade: Optional[int],
        force: bool = False,
    ) -> Dict[str, Any]:
        ...
        existing = self._check_existing_extraction_range(
            endpoint, start_date, end_date, modalidade
        )

        # Build base parameters for date range
        params = {"pagina": 1, "tamanhoPagina": endpoint["default_page_size"]}
        ...
        if not force and existing["success_responses"] > 0:
            ...
            if max_pages > 0 and pages_fetched >= max_pages:
                console.print(
                    f"    ✅ [cyan]Skipping {endpoint['name']} {start_date.date()}-{end_date.date()}"
        ...
        if response_data["status_code"] == 200:
            ...
            console.print(
                f"    🔄 [green]Extracting {endpoint['name']} {start_date.date()}-{end_date.date()}"
                + (f" modalidade={modalidade}" if modalidade else "")
        ...
        self._store_response_range(
            endpoint, params, response_data, start_date, end_date, modalidade
        )
        ...
        if total_pages > 1:
            ...
            concurrent_results = asyncio.run(
                self._fetch_pages_concurrently(
    ...
    async def _extract_endpoint_modalidade_range_async(
        self,
        endpoint: Dict[str, Any],
        start_date: datetime,
        end_date: datetime,
        ...
        if force and existing["success_responses"] > 0:
            console.print(
                f"    🔄 [yellow]Force re-extracting {endpoint['name']} {start_date.date()}-{end_date.date()}"
                + (f" modalidade={modalidade}" if modalidade else "")
    ...
    async def _fetch_pages_concurrently(
        self,
        endpoint: Dict[str, Any],
        ...
        results = {"total_requests": 0, "success_requests": 0, "error_requests": 0}
        ...
        async def fetch_single_page(page_num: int) -> Dict[str, Any]:
            async with semaphore:
                ...
                return {"status_code": response_data["status_code"], "page": page_num}
    ...
    def _extract_endpoint_modalidade(
        self,
        endpoint: Dict[str, Any],
        data_date: datetime,
        modalidade: Optional[int],
        ...
        if force and existing["success_responses"] > 0:
            console.print(
                f"    🔄 [yellow]Force re-extracting {endpoint['name']} {data_date.date()}"
                + (f" modalidade={modalidade}" if modalidade else "")
                + " - overriding existing data[/yellow]"
            )
        ...
        if existing["success_responses"] == 0 or force:
            ...
            if max_pages > 0 and pages_fetched >= max_pages:
                console.print(
                    f"    ✅ [cyan]Skipping {endpoint['name']} {data_date.date()}"
                    + (f" modalidade={modalidade}" if modalidade else "")
                    + f" - already complete ({pages_fetched}/{max_pages} pages)[/cyan]"
                )
                results["skipped"] = True
            ...
            existing_pages = {
                ...
                for row in self.conn.execute(
                    existing_pages_query, existing_params
                ).fetchall()
            }
            missing_pages = [
                p for p in range(1, total_pages + 1) if p not in existing_pages
            ]
            ...
            console.print(
                f"    🔄 [yellow]Resuming {endpoint['name']} {data_date.date()}"
                + (f" modalidade={modalidade}" if modalidade else "")
            ...
            if 1 in missing_pages:
                ...
                self._store_response(
                    endpoint, params, response_data, data_date, modalidade
                )
        ...
        # Get remaining pages
        remaining_pages = (
            list(range(start_page, total_pages + 1))
            if not results["resumed"]
            else missing_pages
        )
    ...
    def _extract_endpoint_modalidade_single(
        self,
        endpoint: Dict[str, Any],
        data_date: datetime,
        modalidade: Optional[int],
    ...
    async def extract_all_data(
        self, start_date: datetime, end_date: datetime, force: bool = False
    ) -> Dict[str, Any]:
        ...
        with Progress(
            ...
            date_range_task = progress.add_task(
                "📅 Processing Date Ranges", total=total_chunks
            )
            ...
            while current_start <= end_date:
                ...
                chunk_end = min(
                    current_start + timedelta(days=364), end_date
                )  # 364 + 1 = 365 days
                days_in_chunk = (chunk_end - current_start).days + 1

                progress.update(
                    date_range_task,
                    description=f"📅 Processing Chunk {chunk_number}: {current_start.date()} to {chunk_end.date()} ({days_in_chunk} days)",
                )
                ...
                await self._extract_chunk_with_progress_bars(
                    chunk_results,
                    total_results,
                    current_start,
                    chunk_end,
    ...
    async def _extract_chunk_with_progress_bars(
        self,
        ...
        for endpoint in PNCP_ENDPOINTS:
            if (
                "modalidades" in endpoint
                and endpoint.get("modalidade_strategy") != "optional_unrestricted"
            ):
        ...
        for pattern in url_patterns:
            ...
            try:
                if endpoint.get("supports_date_range", True):
                    ...
                    result = await self._extract_endpoint_modalidade_range_async(
                        endpoint, start_date, end_date, modalidade, force
                    )
                else:
                    # Use single date extraction for endpoints that don't support ranges
                    result = self._extract_endpoint_modalidade(
                        endpoint, end_date, modalidade, force
                    )
    ...
    async def _discover_pattern_total(
        self,
        ...
        if response_data["status_code"] == 200:
            ...
            url_pattern = f"{endpoint_url}?{'&'.join([f'{k}={v}' for k, v in sample_params.items()])}"
            console.print(
                f"🌐 [dim cyan]{pattern['name']}: {total_pages} pages, {total_records:,} records[/dim cyan]"
            )
            ...
            progress.start_task(task_id)
            console.print(
                f"❌ [red]{pattern['name']}: Failed with status {response_data['status_code']}[/red]"
            )
            return {
                "total_pages": 1,
                "total_records": 0,
    ...
    async def _extract_pattern_pages(
        self,
        pattern: Dict[str, Any],
        start_date: datetime,
        end_date: datetime,
    ...
    def _print_summary(self, results: Dict[str, Any]):
        ...
        total_time = (
            time.time() - self.extraction_start_time
        ...
        if results.get("skipped_extractions", 0) > 0:
            table.add_row(
                "Skipped (Complete)",
                f"{results['skipped_extractions']:,}",
                style="cyan",
            )
        if results.get("resumed_extractions", 0) > 0:
            table.add_row(
                "Resumed (Partial)",
                f"{results['resumed_extractions']:,}",
                style="yellow",
            )

        success_rate = (
            (results["success_requests"] / results["total_requests"] * 100)
        ...
        record_count = self.conn.execute(
            "SELECT COUNT(*) FROM psa.pncp_raw_responses WHERE run_id = ?",
            [results["run_id"]],
        ).fetchone()[0]
        console.print(
            Panel(
    ...
    def get_extraction_stats(self) -> Dict[str, Any]:
        ...
        # Overall stats
        stats["total_responses"] = self.conn.execute(
            "SELECT COUNT(*) FROM psa.pncp_raw_responses"
        ).fetchone()[0]
        stats["unique_endpoints"] = self.conn.execute(
            "SELECT COUNT(DISTINCT endpoint_name) FROM psa.pncp_raw_responses"
        ).fetchone()[0]
        stats["date_range"] = self.conn.execute(
            "SELECT MIN(data_date), MAX(data_date) FROM psa.pncp_raw_responses"
        ).fetchone()

        # Success rate
        success_responses = self.conn.execute(
            "SELECT COUNT(*) FROM psa.pncp_raw_responses WHERE response_code = 200"
        ).fetchone()[0]
        stats["success_rate"] = (
            (success_responses / stats["total_responses"] * 100)
...
def extract(
    ...
    force: bool = typer.Option(
        False, "--force", help="Force re-extraction even if data already exists in PSA"
    ),
):
...
def stats():
    ...
    if stats["date_range"][0]:
        table.add_row(
            "Date Range", f"{stats['date_range'][0]} to {stats['date_range'][1]}"
        )
