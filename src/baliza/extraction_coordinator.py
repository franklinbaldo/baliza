...
class WriterProtocol(Protocol):
    ...
    async def writer_worker(
        self, queue: asyncio.Queue, commit_every: int = 100
    ) -> None:
...
class PlanningPhase(ExtractionPhase):
    ...
    async def _use_existing_plan(
        self, existing_plan: tuple, claimer: TaskClaimer
    ) -> str:
...
class ExecutionPhase(ExtractionPhase):
    ...
    def __init__(
        self, writer: WriterProtocol, shutdown_event: asyncio.Event, concurrency: int
    ):
    ...
    async def _get_pending_task_count(self, plan_fingerprint: str) -> int:
        ...
        with duckdb.connect(self.writer.db_path) as conn:
            ...
            """,
                (TaskStatus.PENDING.value, plan_fingerprint),
            ).fetchone()[0]

    async def _execute_task_batches(
        self, claimer: TaskClaimer, execution_progress, progress: Progress
    ):
    ...
    async def _execute_task_batch(
        self, claimed_tasks: list, claimer: TaskClaimer
    ) -> int:
...
class ExtractionCoordinator:
    ...
    def __init__(
        self, writer: WriterProtocol, concurrency: int = 10, force_db: bool = False
    ):
    ...
    def setup_signal_handlers(self):
        ...
        def signal_handler(signum, frame):
            logger.info(
                f"Received signal {signum} at frame {frame}, initiating graceful shutdown..."
            )
    ...
    async def extract_dbt_driven(
        ...
        claimer = TaskClaimer(
            db_path=self.writer.db_path, worker_id=f"coordinator-{run_id[:8]}"
        )
    ...
    async def _generate_final_report(
        self, start_time: float, claimer: TaskClaimer
    ) -> dict[str, Any]:
        ...
        console.print(
            f"📊 Tasks: {stats['total_tasks']:,} total, {stats['completed_tasks']:,} completed, {stats['failed_tasks']:,} failed"
        )
