...
class ExtractorProtocol(Protocol):
    ...
    async def extract_dbt_driven(
        self, start_date, end_date, use_existing_plan: bool = True
    ):
...
class LoaderProtocol(Protocol):
    ...
    def load(
        self, schemas: list[str] | None = None, max_retries: int = 3
    ) -> dict[str, Any]:
...
class CLIServices:
    ...
    async def run_extraction(
        ...
        extractor_kwargs = {"concurrency": concurrency, "force_db": force_db}

        async with self.container.get_extractor(
            "default", **extractor_kwargs
        ) as extractor:
            if use_dbt:
                return await extractor.extract_dbt_driven(
                    start_date, end_date, not force
                )
            else:
    ...
    def get_extractor(self, name: str = "default", **kwargs) -> ExtractorProtocol:
        """Get an extractor instance."""
        return self.container.get_extractor(name, **kwargs)

    def get_transformer(self, name: str = "default") -> TransformerProtocol:
        """Get a transformer instance."""
        return self.container.get_transformer(name)

    def get_loader(self, name: str = "default") -> LoaderProtocol:
        """Get a loader instance."""
        return self.container.get_loader(name)
...
def create_test_container() -> DependencyContainer:
    ...
    class MockExtractor:
        ...
        async def extract_dbt_driven(
            self, start_date, end_date, use_existing_plan=True
        ):
    ...
    class MockLoader:
        def load(self, schemas=None, max_retries=3):
            ...
            return {
                "export": {"total_files": 5},
                "upload": {"files_uploaded": 5},
                "success": True,
            }
