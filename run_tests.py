...
def main():
    ...
    parser.add_argument(
        "--category",
        choices=["federation", "integration", "notebook", "dbt", "actions", "archive"],
    ...
    if args.category:
        ...
        test_configs.extend(
            [
                {
                    "cmd": base_cmd
                    + ["tests/test_integration.py", "--run-integration"],
                    "description": "Integration Tests",
                },
                {
                    "cmd": base_cmd
                    + ["tests/test_archive_first_flow.py", "--run-integration"],
        ...
        test_configs.extend(
            [
                {
                    "cmd": base_cmd
                    + ["tests/", "-m", "performance", "--run-performance"],
        ...
        test_configs.extend(
            [
                ...
                {
                    "cmd": base_cmd
                    + ["tests/test_archive_first_flow.py::TestArchiveFirstFlow"],
    ...
    print(
        f"🎯 Test mode: {args.category or ('quick' if args.quick else 'full comprehensive')}"
    )
