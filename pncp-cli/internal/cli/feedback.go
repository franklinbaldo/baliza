...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".pncp-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("PNCP_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("PNCP_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "pncp-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.pncp-pp-cli/feedback.jsonl.
When ` + "`PNCP_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`PNCP_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  pncp-pp-cli feedback list
  pncp-pp-cli feedback list --limit 5
  pncp-pp-cli feedback list --json`,
