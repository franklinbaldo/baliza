...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  pncp-pp-cli doctor
  pncp-pp-cli doctor --json
  pncp-pp-cli doctor --fail-on warn`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil {
				...
				if header == "" {
					...
					report["auth_hint"] = "export PNCP_CONSULTA_BEARER_AUTH=<your-key>"
					report["auth_docs_url"] = "https://www.serpro.gov.br"
				} else {
					authConfigured = true
			...
			if os.Getenv("PNCP_CONSULTA_BEARER_AUTH") != "" {
				authEnvSet = append(authEnvSet, "PNCP_CONSULTA_BEARER_AUTH")
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "PNCP_CONSULTA_BEARER_AUTH")
			}
			switch {
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	report := map[string]any{}
	dbPath := defaultDBPath("pncp-pp-cli")
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'pncp-pp-cli sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'pncp-pp-cli sync' to populate."
		return report
	}
	defer rows.Close()
	...
	case !haveAny && len(resources) == 0:
		report["status"] = "unknown"
		report["hint"] = "sync_state is empty; run 'pncp-pp-cli sync' to hydrate."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
		report["oldest_age"] = oldest.Round(time.Minute).String()
		report["hint"] = "Some resources are older than stale_after; run 'pncp-pp-cli sync' to refresh."
	}
	return report
}
