...
In local mode: searches locally synced data only.`,
  ...
  pncp-pp-cli search "error timeout"

  # Force local search only
  pncp-pp-cli search "payment failed" --data-source local

  # Search a specific resource type locally
  pncp-pp-cli search "critical" --type transactions --data-source local

  # JSON output for piping
  pncp-pp-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'pncp-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/pncp-pp-cli/data.db)")
