...
def example_current_vs_ibis_approach():
    ...
    raw_count = con.raw_sql("SELECT COUNT(*) as cnt FROM staging.contratacoes").fetchone()[0]
    ...
    con.create_table("marts.extraction_summary_ibis", extraction_summary, overwrite=True)
    ...
    json_processed = (
        api_requests
        .filter(api_requests.endpoint == "contratacoes_publicacao")
        .mutate([
            # Extract JSON fields using DuckDB JSON functions
            ibis.literal("json_extract(payload_decompressed, '$.totalRegistros')").name("total_registros"),
            ibis.literal("json_array_length(json_extract(payload_decompressed, '$.data'))").name("data_count")
        ])
...
def example_data_quality_with_ibis():
    ...
    data_quality = (
        ...
        .aggregate([
            ...
            (ibis._.collected_at.max() - ibis._.collected_at.min()).name("collection_duration")
        ])
        .order_by([ibis.desc("ingestion_date"), "endpoint"])
    )
...
def example_bulk_operations_with_ibis():
    ...
    temp_table = ibis.memtable(df)
    result = temp_table.to_pandas()  # If needed for processing
